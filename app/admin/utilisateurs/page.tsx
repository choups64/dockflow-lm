...
type Profile = { id: string; email: string; role: Role; magasin_id: string; admin_scope: AdminScope | null; actif: boolean; created_at: string | null; lastSignInAt: string | null; rayonIds: number[] };
...
export default function UtilisateursPage() {
  ...
  const charger = async () => {
    try {
      ...
      const usersResponse = await fetch("/api/admin/users", { headers: { Authorization: `Bearer ${sessionData.session?.access_token ?? ""}` } });
      const usersResult = await usersResponse.json() as { users?: Omit<Profile, "rayonIds">[]; error?: string };
      if (!usersResponse.ok) throw new Error(usersResult.error ?? "Impossible de charger les utilisateurs.");
      const [storesResult, rayonsResult, associationsResult] = await Promise.all([
        storesQuery,
        rayonsQuery,
        supabase.from("profile_rayons").select("profile_id, rayon_id"),
      ]);
      const error = storesResult.error || rayonsResult.error || associationsResult.error;
      ...
      setProfiles((usersResult.users ?? []).map((item) => ({ ...item, role: item.role as Role, admin_scope: item.admin_scope as AdminScope | null, rayonIds: idsByProfile.get(item.id) ?? [] })));
