...
const actionClass = "inline-flex h-10 w-32 items-center justify-center gap-2 rounded-xl border px-3 font-bold disabled:opacity-60";
...
export default function UtilisateursPage() {
  ...
  const storeLabels = useMemo(() => new Map(stores.map((store) => [store.id, `${store.code} — ${store.nom}`])), [stores]);
  const rayonLabels = useMemo(() => new Map(rayons.map((rayon) => [rayon.id, `${rayon.code} — ${rayon.nom}`])), [rayons]);
  ...
  return (
    <>
      ...
      <section className="overflow-x-auto rounded-3xl border border-[#E3E8EC] bg-white shadow-sm">
        <table className="min-w-[1080px] w-full text-left"><thead className="bg-[#F6F8FA] text-sm text-[#66727A]"><tr><th className="p-5">E-mail</th><th className="p-5">Rôle</th><th className="p-5">Magasin</th><th className="p-5">Rayon</th><th className="p-5">Périmètre</th><th className="p-5">Statut</th><th className="w-[420px] p-5 text-right">Actions</th></tr></thead><tbody>{profiles.map((user) => <tr key={user.id} className="border-t border-[#E3E8EC]"><td className="p-5 font-semibold">{user.email}</td><td className="p-5">{user.role}</td><td className="p-5">{storeLabels.get(user.magasin_id) ?? "—"}</td><td className="p-5">{user.rayonIds.length ? user.rayonIds.map((id) => rayonLabels.get(id)).filter(Boolean).join(", ") : "—"}</td><td className="p-5">{user.admin_scope ?? "—"}</td><td className="p-5"><span className={user.actif ? "rounded-full bg-[#EEF7E5] px-3 py-1 text-sm font-bold text-[#4F8F12]" : "rounded-full bg-slate-100 px-3 py-1 text-sm font-bold text-slate-600"}>{user.actif ? "Actif" : "Désactivé"}</span></td><td className="p-5"><div className="flex justify-end gap-2"><button onClick={() => ouvrirEdition(user)} disabled={loading} className={`${actionClass} border-[#E3E8EC] text-[#101820] hover:bg-[#F6F8FA]`}><Pencil size={16} />Modifier</button><button onClick={() => { setSuppression(user); setMessage(null); }} disabled={loading} className={`${actionClass} border-red-200 text-red-700 hover:bg-red-50`}><Trash2 size={16} />Supprimer</button><button onClick={() => void basculer(user)} disabled={loading} className={`${actionClass} border-[#E3E8EC] text-[#101820] hover:bg-[#F6F8FA]`}>{user.actif ? "Désactiver" : "Réactiver"}</button></div></td></tr>)}</tbody></table>
      </section>
