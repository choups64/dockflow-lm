    ...
    <div className="space-y-8">
      ...
      <fieldset disabled={formulaireBloque} className="space-y-8 disabled:opacity-60">
        ...
        <div className="space-y-6">
          {lignes.map((ligne) => <div key={ligne.id} className="relative">
            <button type="button" onClick={() => { setLignes((precedentes) => precedentes.filter((item) => item.id !== ligne.id)); invaliderPreparation(); }} className="absolute right-3 top-3 z-10 inline-flex min-h-10 items-center gap-2 rounded-lg border border-red-200 bg-white px-3 py-2 text-sm font-semibold text-red-600 shadow-sm transition hover:bg-red-50" aria-label="Supprimer cette référence"><Trash2 size={18} />Supprimer</button>
            <PreparationLine reference={ligne.referenceLM} designation={ligne.designation} quantite={ligne.quantite} destinations={destinations} editable modeGlobal={globalCommande} destinationGlobale={destinationGlobale} repartitionsInitiales={ligne.repartitions} onDetailsChange={(details) => mettreAJourLigne(ligne.id, { referenceLM: details.reference, designation: details.designation, quantite: details.quantite })} onChange={({ repartitions }) => mettreAJourLigne(ligne.id, { repartitions })} />
          </div>)}
        </div>
